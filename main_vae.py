...
class ConvVAE(nn.Module):
    def __init__(self, K, input_dims, filter1, filter2):
        ...
        self.p_deconv_2 = nn.ConvTranspose2d(self.filter2, self.filter1, 4, stride=2, output_padding=1)
...
if __name__ == "__main__":
    ...
    reconstructed_sample = vae.get_sample(X_train_sample.to(device))

    # HAVE NOT CHECKED THIS PART -- 
    plt.figure()
    plt.subplot(1,2,0)
    plt.imshow(X_train_sample[0,1,:,:])
    plt.subplot(1,2,1)
    plt.imshow(reconstructed_sample[0,1,:,:].view(X_train_sample.shape))
